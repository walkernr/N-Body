...
##################- Nick Walker -#################
###################- 14/5/2012 -##################
###- Simulates the interactions of celestial -####
########- bodies in a n-body system with  -#######
...
class Celestial(sphere):																																					#the object used for celestial objects
	...
	def __init__(self,radius=1,mass=1e5,density=1,position=vector(0.,0.,0.),velocity=vector(0.,0.,0.),acceleration=vector(0.,0.,0.),rotangle=pi/7500,rotaxis=(0,1,0)):		#the code for the celestial class
	...
	def centripetalVelocity(self,target):																																	#calculate the centripetal velocity of the object relative to a target object
		"""takes target object as an argument
		returns the calculated centripetal velocity of the object about a target object"""
		G=6.67e-11
		v=sqrt(G*target.getMass()/mag(self.pos-target.pos))																													#formula for the magnitude of the velocity
		uv=norm(self.pos-target.pos)																																		#normalized vector between the two objects
...
def collision(bodies,body1,body2,dt):																																		#function for handling the inelastic collision of celestial objects
	...
	if mag(body1.pos-body2.pos)<body1.radius+body2.radius:																													#if the two objects intersect, there is a collision
		collision=True
	if collision==True:
		if body1.getMass()>=body2.getMass() and body2!=bodies[0]:																											#if the first object is larger in mass than the second object, it survives the collision															
	...
	while True:																																								#animation continues forever
		...
		if scene.kb.keys:																																					#capture keystrokes
			...
			if k=='a':																																						#'a' for add planet
				x=randint(int(1.5*bodies[0].radius),int(10*bodies[0].radius))																								#generates x coordinate
				y=randint(-int(bodies[0].radius/4),int(bodies[0].radius/4))																									#generates y coordinate
				z=randint(int(1.5*bodies[0].radius),int(10*bodies[0].radius))																								#generates z coordinate
				position=vector(x,y,z)																																		#constructs position vector out of coordinates
				position=bodies[0].pos+rotate(position,angle=randint(0,360)*pi/180,axis=(0,1,0))																			#rotates position vector about the origin
				planet=Celestial(1,randint(1,1e5),randint(5,15),position,vector(0,0,0),vector(0,0,0),randint(75,125)/100.*pi/7500.,(0,1,0))									#new celestial object created
				...
				if trails==False:																																			#if trails are not in use, retain no points
					planet.retain=0
				planet.setVelocity(planet.centripetalVelocity(star)*randint(75,125)/100.)																					#sets the planet's velocity as 75-125% of its centripetal velocity
			...
			if k=='k':																																						#'k' for i'm running out of letters
				x=randint(0,int(12*bodies[0].radius))																														#generate random radius from star
				y=randint(-int(bodies[0].radius/2),int(bodies[0].radius/2))																									#generate random off-plane y coordinate
				z=sqrt((25*bodies[0].radius)**2-x**2)																														#calculate z coordinate needed for standard radius form star
				position=vector(x,y,z)																																		#construct position vector
				position=bodies[0].pos+rotate(position,angle=randint(0,360)*pi/180,axis=(0,1,0))																			#rotate the position about th origin
				apocalypse=Celestial(1,randint(1e9,1e10),randint(1e5,1e6),position,vector(0,0,0),vector(0,0,0),randint(75,125)/100.*pi/7500.,(0,1,0))						#create apocalyptic space object
				...
				if trails==False:																																			#retain no point in the trail if trail not in use
					apocalypse.retain=0
				velocity=-norm(apocalypse.pos-bodies[0].pos)																												#normalize the vector between the object and the star
				...
				apocalypse.setVelocity(.1*velocity)																															#only one tenth the magnitude of the unit vector
			...
			for target in bodies:																																			#pick an object to calculate vectors for
				for object in bodies:																																		#pick an object to use for the vector calculations
					if target!=object:																																		#check against self-gravity
						if collision(bodies,target,object,dt)==true:																										#check for collisions
						...
						if mag(object.pos-bodies[0].pos)>50*bodies[0].radius:																								#removes planets that leave the system
